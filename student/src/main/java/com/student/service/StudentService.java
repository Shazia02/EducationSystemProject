...
public class StudentService {
	...
	public List<StudentDto> getAllStudent(Long courseId) {
		List<Student> studentList = studentRepository.findByCourseId(courseId);
	...
	public StudentDto allocateCourseToStudent(Long courseId, Long studentId) {
		...
		return studentUtility.convertEntityToDto(student.get());	
	}

	public StudentDto updateCoursesForStudent(List<CourseDto> courseIdList, Long studentId) {
		Optional<Student> student = studentRepository.findById(studentId);
		if(!student.isPresent()) {
			throw  new StudentNotfoundException();
		}
		else {
			for(CourseDto course:courseIdList) {
				student.get().setCourseId(course.getCourseId());
				studentRepository.save(student.get());
			}	
		}
		return studentUtility.convertEntityToDto(student.get());	
